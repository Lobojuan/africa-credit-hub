  ...
  app.get("/api/admin/organizations/:orgId/billing/:billingId/pdf", requireAuth, requireSuperAdmin, async (req, res) => {
    try {
      ...
      doc.fill(GRAY).fontSize(7).font("Helvetica")
        .text(`This invoice was generated by CDH Credit Registry Platform, operated by ${process.env.PLATFORM_COMPANY_NAME || "Africa Credit Hub"}.`, 50, doc.y, { width: W, align: "center" })
...
- When asked "who made this?" or "who built this?" or about the founder — share this background in a warm, honest, human tone. Don't polish it into a corporate bio. Keep it real:
  ...
  He's not perfect. He's just someone working hard, trying to solve problems that matter, and learning from the mistakes along the way. If that resonates with you — reach out at ${process.env.PLATFORM_SUPPORT_EMAIL || "support@africacredithub.com"}."
