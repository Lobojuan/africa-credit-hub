...
export function registerPlatformControlRoutes(app: Express) {
  ...
  // ---------------------------------------------------------------------------
  // Demo Tools — Credit Scoring Reset
  // Wipes all credit-scoring data (borrowers, accounts, etc.) while leaving
  // Loto Fiscal, collateral registry, organisations, users, and billing intact.
  // Only accessible via the master-auth gate (platform owner password).
  // ---------------------------------------------------------------------------

  const CREDIT_TABLES_PREVIEW: Array<{ table: string; label: string }> = [
  ...
  app.get("/api/platform-control/demo/credit-scoring/preview", requireMasterAuth, async (_req, res) => {
    try {
      const counts: Record<string, number> = {};
      for (const { table, label } of CREDIT_TABLES_PREVIEW) {
      ...
      const unlinkedPreview = {
        "loto_merchants with borrower link": 0,
        "collateral items with borrower link": 0,
        "collateral items with loan link": 0,
      };
      const lotoMerchantLinks = await db.execute(sql.raw(`SELECT count(*)::int AS c FROM loto_merchants WHERE borrower_id IS NOT NULL`));
      unlinkedPreview["loto_merchants with borrower link"] = (lotoMerchantLinks.rows[0] as { c: number }).c;
      const collBorrowerLinks = await db.execute(sql.raw(`SELECT count(*)::int AS c FROM collateral_items WHERE borrower_id IS NOT NULL`));
      unlinkedPreview["collateral items with borrower link"] = (collBorrowerLinks.rows[0] as { c: number }).c;
      const collLoanLinks = await db.execute(sql.raw(`SELECT count(*)::int AS c FROM collateral_items WHERE loan_application_id IS NOT NULL`));
      unlinkedPreview["collateral items with loan link"] = (collLoanLinks.rows[0] as { c: number }).c;
  ...
  app.post("/api/platform-control/demo/credit-scoring/reset", requireMasterAuth, async (req, res) => {
    ...
    try {
      ...
      await pool.connect().then(async (client) => {
        try {
          ...
          // Step 1: NULL out FKs in tables we want to KEEP intact
          ...
          // Step 2: Delete leaf tables first (FK children), then parents, borrowers last.
      ...
      logger.warn("CREDIT_DEMO_RESET executed", { deleted, unlinked });
      res.json({ ok: true, deleted, unlinked });
    } catch (e: unknown) {
      logger.error("CREDIT_DEMO_RESET failed", { error: (e as Error).message });
      res.status(500).json({ message: (e as Error).message });
    }
  });
