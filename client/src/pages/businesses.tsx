...
import { Search, Building2, ChevronRight, ChevronLeft, Flag, Factory, Calendar } from "lucide-react";
...
export default function BusinessesPage() {
  ...
  const buildQueryKey = () => {
    if (search) return `/api/businesses?search=${encodeURIComponent(search)}`;
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center flex flex-col items-center gap-3">
            ...
            <h3 className="font-semibold" data-testid="text-no-businesses">
              {search ? "No businesses found" : "No businesses yet"}
            </h3>
            <p className="text-sm text-muted-foreground max-w-xs">
              {search
                ? "Try adjusting your search or clearing the filter."
                : "Register your first corporate borrower to get started."}
            </p>
            {!search && (
              <Button size="sm" onClick={() => navigate("/borrowers")}
                data-testid="button-empty-register-business">
                Register Business
              </Button>
            )}
          </CardContent>
