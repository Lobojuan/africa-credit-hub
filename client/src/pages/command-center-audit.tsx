    ...
    <div className="space-y-4" data-testid="panel-audit-logs">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        {(data?.actionCounts || []).slice(0, 3).map(a => (
          <div key={a.action} className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3 text-center">
            <p className="text-2xl font-bold text-white">{a.count}</p>
            <p className="text-[10px] text-slate-400">{a.action} Events</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <div className="flex items-center gap-1.5">
          ...
          <Select value={filterAction} onValueChange={(v) => { setFilterAction(v); setPage(0); }}>
            <SelectTrigger className="h-8 w-[130px] text-xs bg-slate-800/50 border-slate-700/50 text-white" data-testid="select-audit-action">
              <SelectValue placeholder="Action" />
            </SelectTrigger>
            <SelectContent className="bg-slate-900 border-slate-700">
              <SelectItem value="all" className="text-slate-300 text-xs">All Actions</SelectItem>
              {(data?.actionCounts || []).map(a => (
                <SelectItem key={a.action} value={a.action} className="text-slate-300 text-xs">{a.action} ({a.count})</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        <Select value={filterEntity} onValueChange={(v) => { setFilterEntity(v); setPage(0); }}>
          <SelectTrigger className="h-8 w-[140px] text-xs bg-slate-800/50 border-slate-700/50 text-white" data-testid="select-audit-entity">
            <SelectValue placeholder="Entity" />
          </SelectTrigger>
          <SelectContent className="bg-slate-900 border-slate-700">
            <SelectItem value="all" className="text-slate-300 text-xs">All Entities</SelectItem>
            {(data?.entityCounts || []).map(e => (
              <SelectItem key={e.entity} value={e.entity} className="text-slate-300 text-xs">{e.entity} ({e.count})</SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 overflow-hidden">
        <div className="grid grid-cols-[auto_1fr_auto_auto_auto] gap-0 text-[10px] font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/50 px-3 py-2">
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center text-slate-500 text-sm">No audit events found</div>
        ) : (
          (data?.logs || []).map((log: any) => (
            <div key={log.id} className="grid grid-cols-[auto_1fr_auto_auto_auto] gap-0 items-center border-b border-slate-700/20 px-3 py-2 hover:bg-slate-700/20 transition-colors" data-testid={`audit-row-${log.id}`}>
              ...
              <span className="text-xs text-white truncate pr-2">
                <span className="text-slate-400 mr-1">[{log.entity}]</span>
                {log.details || `${log.action} on ${log.entity}`}
              </span>
              <span className="w-[100px] text-[10px] text-slate-400 truncate">{log.userName}</span>
              <span className="w-[90px] text-[10px] text-slate-500 font-mono">{log.ipAddress || "—"}</span>
              <span className="w-[130px] text-[10px] text-slate-500 text-right">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <Button variant="outline" size="sm" className="h-7 text-xs border-slate-700 text-slate-400" onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} data-testid="button-audit-prev">
            <ChevronLeft className="w-3 h-3" /> Prev
          </Button>
          <span className="text-xs text-slate-500">Page {page + 1} of {totalPages}</span>
          <Button variant="outline" size="sm" className="h-7 text-xs border-slate-700 text-slate-400" onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} data-testid="button-audit-next">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3">
          <p className="text-[10px] text-slate-400 font-semibold mb-2 uppercase tracking-wider">By Action Type</p>
            ...
            {(data?.actionCounts || []).sort((a, b) => b.count - a.count).map(a => {
              ...
              return (
                <div key={a.action} className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-slate-400 font-mono w-[40px] text-right">{a.count}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3">
          <p className="text-[10px] text-slate-400 font-semibold mb-2 uppercase tracking-wider">By Entity Type</p>
          <div className="space-y-1.5">
            {(data?.entityCounts || []).sort((a, b) => b.count - a.count).slice(0, 10).map(e => {
              ...
              return (
                <div key={e.entity} className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-300 w-[80px] truncate">{e.entity}</span>
                  <div className="flex-1 bg-slate-700/30 rounded-full h-2 overflow-hidden">
                    <div className="h-full bg-violet-500/60 rounded-full transition-all" style={{ width: `${(e.count / maxCount) * 100}%` }} />
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono w-[40px] text-right">{e.count}</span>
                </div>
              );
            })}
          </div>
