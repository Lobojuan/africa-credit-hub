...
function QualityBar({ label, total, missing, color }: { label: string; total: number; missing: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-slate-300 w-[100px] truncate">{label}</span>
      ...
      <span className="text-[9px] text-slate-500 w-[60px] text-right">{missing} missing</span>
    </div>
  );
}
    ...
    <div className="space-y-4" data-testid="panel-data-quality">
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3 text-center">
          ...
          <p className="text-[10px] text-slate-400">Overall Completeness</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3 text-center">
          <p className="text-2xl font-bold text-white">{data?.borrowers.total ?? "—"}</p>
          <p className="text-[10px] text-slate-400">Borrowers</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3 text-center">
          <p className="text-2xl font-bold text-white">{data?.accounts.total ?? "—"}</p>
          <p className="text-[10px] text-slate-400">Credit Accounts</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3 text-center">
          <p className="text-2xl font-bold text-white">{data?.relatedEntities.payments ?? "—"}</p>
          <p className="text-[10px] text-slate-400">Payment Records</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3 text-center">
          <p className="text-2xl font-bold text-white">{data?.byCountry.length ?? "—"}</p>
          <p className="text-[10px] text-slate-400">Countries w/ Data</p>
        </div>
      </div>

      {isLoading ? (
        <div className="text-center text-slate-500 text-sm py-8">Analyzing data quality...</div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-4">
              ...
              <div className="mt-4 pt-3 border-t border-slate-700/30">
                <p className="text-[10px] text-slate-400 font-semibold mb-2 uppercase tracking-wider">Related Entity Coverage</p>
          ...
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-4">
            ...
            {(data?.byCountry || []).length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-4">No country-level data available</p>
            ) : (
                ...
                {data!.byCountry.sort((a, b) => b.count - a.count).map(c => {
                  ...
                  return (
                    <div key={c.country} className="flex items-center gap-2">
                      <span className="text-[10px] text-slate-300 w-[100px] truncate">{c.country}</span>
                      ...
                      <span className="text-[10px] text-slate-400 font-mono w-[50px] text-right">{c.count.toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
