...
function StatCard({ icon: Icon, label, value, sub, color }: {
  ...
  return (
    <div className="rounded-lg border border-border bg-foreground/40 p-3" data-testid={`stat-${label.toLowerCase().replace(/\s/g, '-')}`}>
    ...
    <div className="border border-border/30 rounded-lg overflow-hidden">
      ...
      {open && (
        <div className="border-t border-border/30">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3 px-3 py-2 border-b border-border/20 last:border-0 hover:bg-white/10/10">
    ...
    <div className="space-y-4" data-testid="system-infrastructure-tab">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="outline"
          size="sm"
          className="h-7 text-[10px] border-border text-muted-foreground hover:text-white gap-1"
      ...
      <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            ...
            <div key={label} className="flex items-center gap-2 p-2 rounded-lg bg-white/5 border border-border/30">
