...
| R-07 | Monitor CAR, liquidity, deposit concentration, impairment, and regulatory breaches before they become sanctions. | **Partial** | Regulatory dashboards, reporting exports, Portfolio Intelligence, platform metrics. | Add Funding & Prudential Radar with daily concentration/runoff signals, CAR/liquidity feeds, scenario alerts, and monthly recovery-report workflow. Target: no missed in-scope filing deadline or pre-breach alert. |
...
| R-09 | Reduce regulatory reporting workload and produce examiner-ready evidence. | **Partial** | BoG/CBN/CBK/BSL export modules, audit trail, data-quality controls, regulatory compliance views. | Add directive-to-control mapping, filing calendar/escalations, validation evidence, wilful-defaulter workflow, and one-click evidence packs. Target: zero late or materially incorrect pilot filings. |
| R-10 | Provide local, resolution-first customer support without a misleading chatbot. | **Planned** | AI Command Center, Helpdesk, SMS/USSD foundations. | Build narrow customer journeys first (failed transaction, account freeze, complaint status), retrieval-lock answers to approved bank content, require human handoff for exceptions, support English and Twi. Measure resolution and CSAT, not deflection. |
