...
-   **Core Capabilities**:
    ...
    -   **Client Landing Page (Default Landing)**: Client-focused institutional page (`/solutions`, also `/investor`, `/demo`) — serves as the default landing page for unauthenticated users (visiting `/` redirects to `/solutions`). Nav bar has "Log In" button (→ `/login`) and "Start Free Trial" button (→ `/start-trial`). Includes AI-generated images, animated counters, platform module deep-dive (16 modules), institutional use cases (central banks, commercial banks, DFIs, MFIs/fintechs), competitor comparison (TransUnion, Experian, country bureaus, in-house), market proof section with demand signals, security highlights, and 3-step onboarding flow.
    -   **Login Page**: Dedicated login page at `/login` with username/password form, MFA support, "Back to home" link (→ `/solutions`), and "Start free trial" link (→ `/start-trial`). After successful login, redirects to dashboard (`/`). Previously the login form was shown inline at `/` for unauthenticated users.
    -   **Trial Registration**: Self-service registration page (`/start-trial`) with 2-step form: Step 1 collects organization details (name, type, country, contact email/phone), Step 2 collects admin credentials (full name, email, username, password). Creates organization with "trial" subscription tier and admin user. Rate-limited (5 attempts per 15 min). Auto-logs in after registration. Seeds 10 sample borrowers (across 7 African countries), 14 credit accounts (diverse types/currencies/statuses), sample disputes, and audit trail entries. Module: `server/trial-sandbox.ts`. API: `POST /api/trial/register`.
    -   **Trial Banner**: Blue banner at top of authenticated app for trial users showing upgrade CTA. Dismissible. Component: `client/src/components/trial-banner.tsx`.
    -   **Upgrade & Payment Page**: Self-service upgrade page (`/upgrade`) with 3-step flow: plan selection → payment method → confirmation. Supports 5 payment methods: Stripe (credit/debit cards), Bank/Wire Transfer (with copyable bank details), Flutterwave (pan-African processor for 30+ countries), M-Pesa (East African mobile money with STK push), Mobile Money (MTN MoMo, Airtel Money, Orange Money). Creates billing records and audit logs for each payment. API: `POST /api/payments/initiate`.
    ...
    -   **Market Validation Page**: Public-facing page (`/market-validation`) with data-backed market analysis: TAM by region (2,700+ institutions across 5 African regions), 6 sourced demand signals, competitive landscape analysis (TransUnion, Experian, CRB Africa, XDS Data, in-house systems), pricing validation against African SaaS benchmarks, traction milestones, and "Why Now" convergence analysis. Linked from investor page navigation and revenue model CTA.
