...
-   **Core Capabilities**:
    ...
    -   **Platform Command Center**: Super admin landing page (`client/src/pages/country-selection.tsx`) with 3 tabs: (1) Jurisdictions — 10 country cards with live stats, compliance badges, and SATA indicators; (2) Compliance & SATA — data protection law status, SATA cross-border readiness, SRS requirements traceability by category; (3) Feature Matrix — table of capabilities per country (credit scoring, regulatory export, dispute mgmt, consent, batch upload, API, KYC, SATA). API endpoint: `GET /api/platform/command-center` (optimized batch SQL, no N+1 queries).
