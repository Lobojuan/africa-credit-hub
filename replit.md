...
-   **Core Capabilities**:
    ...
    -   **Platform Command Center**: Super admin landing page (`client/src/pages/country-selection.tsx`) with 5 tabs: (1) Jurisdictions — 10 country cards with live stats, compliance badges, and SATA indicators; (2) Compliance & SATA — data protection law status, SATA cross-border readiness, SRS requirements traceability by category; (3) Feature Matrix — table of capabilities per country (credit scoring, regulatory export, dispute mgmt, consent, batch upload, API, KYC, SATA); (4) Users & Clients — full user CRUD (search/filter by role/status/country, create/edit/delete users) and organization management (search, onboard new clients, delete orgs) with sub-tabs; (5) Country Settings — country selector with per-country feature configuration display, SATA agreement management (create/activate/suspend/delete bilateral agreements with data type selection), and country metadata display. Components: `command-center-users.tsx`, `command-center-settings.tsx`. API endpoint: `GET /api/platform/command-center` (optimized batch SQL, no N+1 queries).
