...
-   **Core Capabilities**:
    ...
    -   **Telco Credit Scoring**: Dedicated AI-driven mobile money (MoMo) analytics section for credit-scoring unbanked/underbanked populations. Three tables: `telcoProfiles` (subscriber info with MSISDN, provider, KYC level), `momoTransactions` (transaction records with type, amount, counterparty), `telcoCreditScores` (AI-generated risk assessments). Backend: `server/telco-scoring.ts` computes 90-day financial/telemetric/network/risk KPIs and generates structured AI credit scores (1-5 risk scale, tier classification, credit limit recommendations). Frontend: `/telco-scoring` page with tabbed dashboard (Analytics & ROI, Profiles, Scores). Demo seed creates 19 profiles across 7 countries (Ghana, Kenya, Uganda, Tanzania, Rwanda, Sierra Leone, Nigeria) with pre-computed AI scores and full KPI snapshots. Analytics dashboard shows: financial inclusion impact (unbanked %, women/rural borrowers), ROI & unit economics (cost/revenue per score, gross margin, NPL reduction, default savings, annualized ROI), model performance (accuracy, Gini coefficient, KS statistic, false positive/negative rates), country performance table, risk tier distribution bars, and KYC level breakdown. API: `GET /api/telco/analytics` returns comprehensive platform analytics.
