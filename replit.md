...
-   **Core Capabilities**:
    ...
    -   **Trial Registration**: Self-service registration page (`/start-trial`) with 2-step form: Step 1 collects organization details (name, type, country, contact email/phone), Step 2 collects admin credentials (full name, email, username, password). Creates organization with "trial" subscription tier and admin user. Rate-limited (5 attempts per 15 min). Auto-logs in after registration. API: `POST /api/trial/register`.
