...
-   **Borrower Record Merge** (v2.6): Deduplication tool for admin/super_admin roles — `POST /api/borrowers/merge` re-points all FK child records (credit_accounts, credit_inquiries, payment_history, disputes, court_judgments, dishonoured_cheques, consent_records, alternative_data, guarantors, portfolio triggers, consumer monitoring, identity_verifications, watchlist_hits, fraud_alerts, audit_logs, borrower_alerts) from duplicate borrowers to the primary, fills missing profile fields on primary from duplicates, then permanently deletes the duplicates. Preview endpoint: `GET /api/borrowers/merge/preview?ids=...` returns borrower details + account counts per record. UI: toggle "Select to Merge" in credit search results (consumer/business tabs), click checkboxes to select 2+ records, click "Merge X Records" → preview dialog with primary selector → confirm merge → navigates to merged record.
