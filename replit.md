...
-   **Ghana Compliance Documentation Hub**: Dedicated `/ghana-docs` page (Ghana mode only) with 6 BoG compliance documents in `docs/ghana/` — SLA Agreement, Compliance Framework, E2E Test Plan, Data Standards, Data Protection Policy, Operational Procedures. API routes at `/api/ghana-docs` (list), `/api/ghana-docs/:id` (view with HTML), `/api/ghana-docs/:id/pdf` (PDF download via pdfkit), `/api/ghana-docs/:id/download` (markdown download). Frontend page with category filtering, inline document viewer (Dialog), and PDF/markdown download buttons. Sidebar entry visible in Ghana mode only.
