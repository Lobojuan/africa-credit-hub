...
-   **Core Capabilities**:
    ...
    **Enhanced API Developer Portal**: Interactive sandbox, webhook event documentation.
    -   **Security Hardening**: Helmet security headers (CSP, HSTS with preload, X-Frame-Options: DENY, Referrer-Policy, Permissions-Policy), DOMPurify sanitization, CSRF token protection on all state-changing requests, AES-256-GCM PII encryption at rest (national IDs, dates of birth, mobile money numbers via `server/encryption.ts`), data subject erasure request API with dual-approval workflow.
