...
-   **Core Capabilities**:
    ...
    -   **Partner Documentation Page**: Public page at `/partner-docs` with comprehensive documentation for bureaus, banks, MFIs, and fintechs. Covers platform overview, who should connect, per-transaction pricing with volume tiers, prepaid wallet system, accepted payment methods (MTN MoMo, Vodafone Cash, AirtelTigo, 18 Ghana banks, Stripe), revenue split model, 6-step onboarding process, technical integration requirements, compliance/security details, SLA tiers, FAQ, and contact information. Linked from the pricing page.
