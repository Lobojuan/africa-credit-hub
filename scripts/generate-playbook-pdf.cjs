/**
 * UCH Demo Playbook PDF Generator
 *
 * Usage:
 *   node scripts/generate-playbook-pdf.cjs <market>   # single market
 *   node scripts/generate-playbook-pdf.cjs            # all markets
 *
 * Supported market keys: ghana, nigeria, kenya, civ, egypt, ethiopia
 *
 * Input:  exports/<market>-demo-playbook.md
 * Output: exports/<market>-demo-playbook.pdf
 *
 * To add a new market: add an entry to MARKET_CONFIG below and create the
 * corresponding markdown source file in exports/.
 */
...
const MARKET_CONFIG = {
  ghana: {
    mdFile: "ghana-demo-playbook.md",
    pdfFile: "ghana-demo-playbook.pdf",
    title: "Ghana Demo Playbook",
    subtitle: "Universal Credit Hub — Pan-African Credit Registry",
    tagline: "Confidential  ·  May 2026  ·  Presentation Day: 11:00 AM GMT",
    pdfTitle: "Ghana Demo Playbook — Universal Credit Hub",
    pdfSubject: "Ghana market demonstration playbook and talking points",
    pdfKeywords: "Ghana, credit bureau, UCH, demo, playbook, Bank of Ghana",
  },
  nigeria: {
    mdFile: "nigeria-demo-playbook.md",
    pdfFile: "nigeria-demo-playbook.pdf",
    title: "Nigeria Demo Playbook",
    subtitle: "Universal Credit Hub — Pan-African Credit Registry",
    tagline: "Confidential  ·  May 2026  ·  Presentation Day: 11:00 AM WAT",
    pdfTitle: "Nigeria Demo Playbook — Universal Credit Hub",
    pdfSubject: "Nigeria market demonstration playbook and talking points",
    pdfKeywords: "Nigeria, credit bureau, UCH, demo, playbook, CBN, eNaira, BVN",
  },
  kenya: {
    mdFile: "kenya-demo-playbook.md",
    pdfFile: "kenya-demo-playbook.pdf",
    title: "Kenya Demo Playbook",
    subtitle: "Universal Credit Hub — Pan-African Credit Registry",
    tagline: "Confidential  ·  May 2026  ·  Presentation Day: 11:00 AM EAT",
    pdfTitle: "Kenya Demo Playbook — Universal Credit Hub",
    pdfSubject: "Kenya market demonstration playbook and talking points",
    pdfKeywords: "Kenya, credit bureau, UCH, demo, playbook, CBK, M-Pesa, CRB, KDPA",
  },
  civ: {
    mdFile: "civ-demo-playbook.md",
    pdfFile: "civ-demo-playbook.pdf",
    title: "Côte d'Ivoire Demo Playbook",
    subtitle: "Universal Credit Hub — Pan-African Credit Registry",
    tagline: "Confidentiel  ·  Mai 2026  ·  Présentation : 11:00 AM GMT",
    pdfTitle: "Côte d'Ivoire Demo Playbook — Universal Credit Hub",
    pdfSubject: "Côte d'Ivoire market demonstration playbook and talking points",
    pdfKeywords: "Cote d'Ivoire, BCEAO, UEMOA, UCH, demo, playbook, Loto Fiscal, DGI, XOF",
  },
  egypt: {
    mdFile: "egypt-demo-playbook.md",
    pdfFile: "egypt-demo-playbook.pdf",
    title: "Egypt Demo Playbook",
    subtitle: "Universal Credit Hub — Pan-African Credit Registry",
    tagline: "Confidential  ·  May 2026  ·  Presentation Day: 11:00 AM EET",
    pdfTitle: "Egypt Demo Playbook — Universal Credit Hub",
    pdfSubject: "Egypt market demonstration playbook and talking points",
    pdfKeywords: "Egypt, credit bureau, UCH, demo, playbook, CBE, Fawry, EGP, PDPL, Arabic",
  },
  ethiopia: {
    mdFile: "ethiopia-demo-playbook.md",
    pdfFile: "ethiopia-demo-playbook.pdf",
    title: "Ethiopia Demo Playbook",
    subtitle: "Universal Credit Hub — Pan-African Credit Registry",
    tagline: "Confidential  ·  May 2026  ·  Presentation Day: 11:00 AM EAT",
    pdfTitle: "Ethiopia Demo Playbook — Universal Credit Hub",
    pdfSubject: "Ethiopia market demonstration playbook and talking points",
    pdfKeywords: "Ethiopia, credit bureau, UCH, demo, playbook, NBE, CBE Birr, telebirr, ETB",
  },
};
...
function addPageNumbers(doc, market) {
...
function generatePlaybookPdf(marketKey) {
  ...
  const mdPath = path.join(__dirname, "../exports", config.mdFile);
  const outPath = path.join(__dirname, "../exports", config.pdfFile);
  ...
  doc.font("Helvetica-Bold").fontSize(26).fillColor("#ffffff")
    .text(config.title, PAGE_MARGINS.left, 30, { width: pageWidth });
  doc.font("Helvetica").fontSize(13).fillColor("#ccf5f2")
    .text(config.subtitle, PAGE_MARGINS.left, 62, { width: pageWidth });
  doc.font("Helvetica").fontSize(10).fillColor("#99e0db")
    .text(config.tagline, PAGE_MARGINS.left, 84, { width: pageWidth });
  ...
  addPageNumbers(doc, marketKey);
  doc.end();

  out.on("finish", () => {
    const size = fs.statSync(outPath).size;
    console.log(`PDF generated: ${outPath} (${(size / 1024).toFixed(1)} KB)`);
  });

  out.on("error", (err) => {
    console.error(`Error writing PDF for ${marketKey}:`, err);
    process.exit(1);
  });
}
...
if (!arg) {
  console.log("Usage: node generate-playbook-pdf.cjs <market>");
  console.log("Markets: " + Object.keys(MARKET_CONFIG).join(", "));
  console.log("\nGenerating all markets...");
  for (const key of Object.keys(MARKET_CONFIG)) {
    generatePlaybookPdf(key);
  }
} else {
  generatePlaybookPdf(arg.toLowerCase());
}
